...
export class SearchBarKaufenV2 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            value: '',
    ...
    _getObjForRequest(selectedFilterOptions, devices, searchType, value, deviceName) {
        ...
        let arrKeys = ['lagerort', 'modell', 'deviceName', 'webShopCategoryIds', 'modelCategoryIds', 'price', 'zustand', 'garantie', 'sort', 'page', 'criterias', 'specifications', 'searchQuery']
        for (let key in objForRequest) {
    ...
    onSuggestionSelected = (event, { suggestion }) => {
        ...
        if (suggestion.searchType === "product") {
            ...
            let modelName = suggestion.name.replace(/ /g, '-').toLowerCase(),
                color = suggestion.color, capacity = suggestion.capacity, deviceName = suggestion.deviceName;
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(SearchBarKaufenV2))
