...
const OtherProducts = ({similarItems, basketActions, basketData, wishlistData}) => {
    ...
    const addToBasket = (e,item) => {
        ...
        if (status === 'out') {
            if (!window.isMobile) {
                const img = item.colorImage ? item.colorImage : '/images/design/Product.svg',
    ...
    const handleOpenDetailPage = (deviceName, modelName, capacity, color, shortcode) => {
        window.open(`//${window.location.host}/kaufen/detail/${deviceName}/${modelName}/${capacity}/${color}/${shortcode}`, '_self')
    }

    const mapSimilarItems = (model, i) => {
        const itemWrap = {paddingLeft: "8px", paddingRight: "8px"};
        const realPrice = model.discountPrice ? model.discountPrice : model.price
        const monthPrice = (realPrice / 12).toFixed(2)
        let description = ''
        if (description != '')
            description += ', '
                    ...
                    <div className='item' onClick={() => handleOpenDetailPage(deviceName, modelName, capacity, color, model.shortcode)}>
                        <div className="img">
                            ...
                            <i className={productIsAddedToWishlist ? 'modelInfoBlock-img-small-wishBtn on' : 'modelInfoBlock-img-small-wishBtn'}
                                     onClick={(e) => addModelToWishlist(e, model)}>
                        ...
                        <div className="heading">
                            ...
                            <div className="colors">
                                <span className="colorPic" style={{backgroundColor: model.colorCode }}/>
                            </div>
                        </div>
    ...
    return(
        <div className='col-xs-12 other-product'>
            <h3>Folgendes könnte Sie auch interessieren</h3>
            <Slider {...settings}>
